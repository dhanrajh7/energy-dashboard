...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function createOrUpdateMeterCard(meter, liveData, now, cardCol) {
    ...
    if (liveData) {
      ...
      // Evaluate alerts for this meter using DB rules
  ...
  -------------------------------------------------------------- */
  let dbAlertRules = [];      // /api/alerts/rules
  let dbActiveTriggered = []; // /api/alerts/triggered (IsActive=1)
  ...
  // Clear Alerts => mark all active triggers as recovered in DB
  clearAlertsBtn.addEventListener('click', async () => {
    const uniqAlertIds = [...new Set(dbActiveTriggered.map(a => a.AlertID))];
    await Promise.all(
      uniqAlertIds.map(id => fetch(`/api/alerts/triggered/recover/${id}`, { method: 'PUT' }))
    );
    await loadTriggeredAlerts();
  });

  // Evaluate rules for a meter with latest live data
  async function checkAlerts(meterId, liveData) {
    if (!liveData) return;

    const rules = dbAlertRules.filter(r => r.IsActive === 1 && r.MeterID == meterId);
    for (const r of rules) {
      const value = liveData[r.Parameter];
      if (value == null) continue;

      // authoritative view of active rows
      const res = await fetch('/api/alerts/triggered');
      const activeRows = await res.json();
      const isActiveInDB = activeRows.some(a => a.AlertID === r.AlertID && a.IsActive === 1);

      if (Number(value) > Number(r.Threshold)) {
        // exceed → trigger if not already active
        if (!isActiveInDB) {
          await fetch('/api/alerts/triggered', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ AlertID: r.AlertID, MeterID: meterId, Message: r.Message })
          });
          showToastAlert(r.Message);
        }
      } else {
        // recovered → set inactive if currently active
        if (isActiveInDB) {
          await fetch(`/api/alerts/triggered/recover/${r.AlertID}`, { method: 'PUT' });
        }
      }
    }

    await loadTriggeredAlerts(); // refresh UI once after all checks
  }
  ...
  (async () => {
    await loadAlertRules();
    await loadTriggeredAlerts();
    await fetchMeters(); // starts the 5s auto-refresh and calls checkAlerts()
  })();
});
